...
public class  UrlQueryParser  extends EvalFunc <Map<String,String>> {
 
public Map<String,String> exec(Tuple input) throws IOException {  
	try{
		String url = (String) input.get(0);
		Map<String, String> output = getQueryMap(url);
		return output;
	}
	catch(Exception e) {}
	return null;
...
public static Map<String, String> getQueryMap(String url) throws MalformedURLException {  
	...
	*/
	...
	map.put("host",urla.getHost());
	map.put("path",urla.getPath());
	map.put("url", url);
	//query = urla.getQuery();
    ...
    Matcher m = re.matcher(query);
      ...
      for (String param : s){
			if(param.indexOf("=") != -1){
				...
				map.put(name, decodedValue.toString());				
			}
		}
	}else{
		map.put("notValid", url);
	...
	public static void main(String[] args) throws MalformedURLException, UnsupportedEncodingException {
		String testUrl = "http://www.google.se?=bar&foo=";
		//String testUrl = "http://www.google.fi/url?sa=t&rct=j&q=&esrc=s&source=web&cd=4&ved=0CGsQFjAD&url=http%253A%252F%252Fwww.tjareborg.fi%252Fportugali%252Flissabon&ei=CsoeU5-jDuK84ASuz4GQBw&usg=AFQjCNGUuSJssjWdqt9ZfbuiBVRysx8OrA&bvm=bv.62788935,d.bGE&v_pt=/Geographical/ResortInfo";
		...
		//System.out.println(URLEncoder.encode(testUrl, "UTF-8"));
		
		//Map<String, String> map = getQueryMap(testUrl);
		System.out.println(getQueryMap(testUrl));
		/*

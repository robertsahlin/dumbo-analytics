...
-- IF 8 datanodes, set default_parallel to 10 (12 available slots, but reserve 2 for retries)
--set HCAT_HOME=c:\apps\dist\hcatalog-0.11.0.1.3.1.0-06
--set PIG_CLASSPATH=%HCAT_HOME%\share\hcatalog\hcatalog-*.jar;%HIVE_HOME%\lib\hive-metastore-*.jar;%HIVE_HOME%\lib\libthrift-*.jar;%HIVE_HOME%\lib\hive-exec-*.jar;%HIVE_HOME%\lib\libfb303-*.jar;%HIVE_HOME%\lib\jdo2-api-*-ec.jar;%HIVE_HOME%\conf;%HADOOP_HOME%\conf;%HIVE_HOME%\lib\slf4j-api-*.jar
...
UNION_PE = UNION onschema EVENTS,PAGEVIEWS;

TRIGGER_REDUCE = GROUP UNION_PE BY session_id;
ROWS_FOR_STORAGE = FOREACH TRIGGER_REDUCE {
    GENERATE FLATTEN(UNION_PE) AS (dt, visitor_id, session_id, time, tech, meta, page, referrer, flags, date);
};
...
--B = ORDER ROWS_FOR_STORAGE BY dt PARALLEL 1;
